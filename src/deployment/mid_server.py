...
class MIDServerDeployer:
    ...
    def deploy(self):
        ...
        """
        self.logger.info(
            f"Starting MID server deployment for environment: {self.environment}"
        )
        ...
        try:
            ...
            self._setup_ecs_service(
                cluster_name, task_definition_arn, subnet_ids, [sg_id]
            )

            self.logger.info(
                f"MID server deployment completed for environment: {self.environment}"
            )
    ...
    def _setup_network(self) -> tuple:
        ...
        try:
            ...
            if not vpcs["Vpcs"]:
                self.logger.error(
                    "No VPCs found. Please create a VPC before deploying."
                )
            ...
            self.logger.info(
                f"Using VPC: {vpc_id} with subnets: {', '.join(subnet_ids)}"
            )
    ...
    def _setup_iam_roles(self) -> tuple:
        ...
        try:
            ...
            task_role = self._create_or_get_role(
                task_role_name, "ecs-tasks.amazonaws.com"
            )
            ...
            self.logger.info(
                f"Set up IAM roles: {task_role_name}, {execution_role_name}"
            )
